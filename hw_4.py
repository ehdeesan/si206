...
class Customer:
    ...
    def make_order(self, server, amount):
        self.wallet -= (amount + (0.20*amount)) # Deducting tip
        server.receive_payment(amount + (0.20*amount)) # Adding tip
...
class Truck:
    ...
    def has_food(self, food_item, quantity):
        remaining = self.inventory.get(food_item, 0)
    ...
    def __str__(self):
        ...
        for key, val in self.inventory.items():
            lst.append(key)
        return "Hello, we are " + self.name + ". This is the current menu " + str(', '.join(lst)) + ". We charge $" + str(self.cost) + " per item. We have $" + str(self.money) + " in total."
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_make_payment(self):
        ...
        tip = 0.20*30
        self.assertEqual(self.f2.wallet, previous_wallet_customer - 30 - tip)
        self.assertEqual(self.s2.money, previous_money_server + 30 + tip)
...
def main():
    truck_1_inven = {'burger': 10, 'nachos': 2, 'fries': 27}
    truck_2_inven = {'noodles': 5, 'dumplings': 10, 'rice': 50}
    cust_1 = Customer('Eddie', 150)
    cust_2 = Customer('Bob', 100)
    American = Truck('American', truck_1_inven, 10, 500)
    Asian = Truck('Asian', truck_2_inven, 15, 600)
    serv_1 = Server('Jack', 200, [American,Asian], 10)
    serv_2 = Server('Jill', 150, [Asian], 8)
